...
cmp -> operand _ GTE _ operand {% ([left,, op,, right]) => ({ left, op: "GTE", right }) %}
     | operand _ LTE _ operand {% ([left,, op,, right]) => ({ left, op: "LTE", right }) %}
...
dictLiteral -> LBRACE ___ dictBody ___ RBRACE {% ([,,entries]) => ({ type: "DICTIONARY", entries }) %}
